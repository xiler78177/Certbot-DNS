...
wg_rebuild_conf() {
...

    {
        ...
        echo "PostUp = iptables -A FORWARD -i %i -j ACCEPT; iptables -A FORWARD -o %i -j ACCEPT; iptables -t nat -A POSTROUTING -o ${main_iface} -j MASQUERADE"
        echo "PostDown = iptables -D FORWARD -i %i -j ACCEPT; iptables -D FORWARD -o %i -j ACCEPT; iptables -t nat -D POSTROUTING -o ${main_iface} -j MASQUERADE"
        ...
        while [[ $i -lt $pc ]]; do
            if [[ "$(wg_db_get ".peers[$i].enabled")" == "true" ]]; then
                ...
                echo "AllowedIPs = $(wg_db_get ".peers[$i].ip")/32"
            fi
            i=$((i + 1))
        done
    } > "$WG_CONF"
...
wg_regenerate_client_confs() {
    ...
    while [[ $i -lt $pc ]]; do
        ...
        write_file_atomic "/etc/wireguard/clients/${name}.conf" "[Interface]
...
wg_server_install() {
    ...
    local wg_port
    while true; do
        read -e -r -p "WireGuard 监听端口 [51820]: " wg_port
...
PersistentKeepalive = 25"
    ...
    log_action "WireGuard peer added: ${peer_name} (${peer_ip})"
    pause
}
...
wg_list_peers() {
    ...
    printf "${C_CYAN}%-4s %-16s %-16s %-8s %-12s %-12s %s${C_RESET}\n" \
        "#" "名称" "IP" "状态" "↓接收" "↑发送" "最近握手"
    draw_line
    ...
    while [[ $i -lt $peer_count ]]; do
        ...
        printf "%-4s %-16s %-16s %-8b %-12s %-12s %s\n" \
            "$((i + 1))" "$name" "$ip" "$status_str" \
...
wg_toggle_peer() {
    ...
    local i=0
    while [[ $i -lt $peer_count ]]; do
        local name ip enabled status_icon
        name=$(wg_db_get ".peers[$i].name")
        ip=$(wg_db_get ".peers[$i].ip")
        enabled=$(wg_db_get ".peers[$i].enabled")

        if [[ "$enabled" == "true" ]]; then
            status_icon="${C_GREEN}● 启用${C_RESET}"
        else
            status_icon="${C_RED}○ 禁用${C_RESET}"
        fi

        echo -e "  $((i + 1)). ${name} (${ip}) - ${status_icon}"
        i=$((i + 1))
    done
    echo "  0. 返回"
    echo ""
    ...
    if [[ "$current_state" == "true" ]]; then
        ...
        wg_is_running && wg set "$WG_INTERFACE" peer "$target_pubkey" remove 2>/dev/null || true
        wg_rebuild_conf
...
wg_add_port_forward() {
    ...
    if [[ "$proto" == "tcp+udp" ]]; then
        _wg_apply_pf_rules "tcp"
        _wg_apply_pf_rules "udp"
    else
        _wg_apply_pf_rules "$proto"
    fi
...
wg_delete_port_forward() {
    ...
    if [[ "$proto" == "tcp+udp" ]]; then
        _wg_remove_pf_rules "tcp"
        _wg_remove_pf_rules "udp"
    else
        _wg_remove_pf_rules "$proto"
    fi
...
wg_restore_port_forwards() {
    ...
    while [[ $j -lt $pf_count ]]; do
        ...
        if [[ "$pf_enabled" == "true" ]]; then
            _wg_restore_pf() {
                local p=$1
                if ! iptables -t nat -C PREROUTING -i "$main_iface" -p "$p" --dport "$ext_port" -j DNAT --to-destination "${dest_ip}:${dest_port}" 2>/dev/null; then
                    iptables -t nat -A PREROUTING -i "$main_iface" -p "$p" --dport "$ext_port" -j DNAT --to-destination "${dest_ip}:${dest_port}" 2>/dev/null
                fi
                if ! iptables -C FORWARD -i "$main_iface" -o "$WG_INTERFACE" -p "$p" --dport "$dest_port" -d "$dest_ip" -j ACCEPT 2>/dev/null; then
                    iptables -A FORWARD -i "$main_iface" -o "$WG_INTERFACE" -p "$p" --dport "$dest_port" -d "$dest_ip" -j ACCEPT 2>/dev/null
                fi
            }
    ...
    if [[ "$role" == "server" ]]; then
        ...
        if [[ -n "$pf_count" && "$pf_count" -gt 0 ]] 2>/dev/null; then
            ...
            while [[ $j -lt $pf_count ]]; do
                ...
                _wg_clean_pf() {
                    local p=$1
                    iptables -t nat -D PREROUTING -i "$main_iface" -p "$p" --dport "$ext_port" -j DNAT --to-destination "${dest_ip}:${dest_port}" 2>/dev/null || true
                    iptables -D FORWARD -i "$main_iface" -o "$WG_INTERFACE" -p "$p" --dport "$dest_port" -d "$dest_ip" -j ACCEPT 2>/dev/null || true
                }
...
wg_server_menu() {
    while true; do
        ...
        case $choice in
            ...
            4) wg_toggle_peer ;;
            5) wg_show_peer_conf ;;
            6) wg_port_forward_menu ;;
            7) wg_start; pause ;;
            8) wg_stop; pause ;;
            9) wg_restart; pause ;;
            10) wg_modify_server ;;
            11) wg_uninstall; return ;;
            0|"") return ;;
            *) print_warn "无效选项" ;;
        esac
    done
