...
#
# 更新日志 (V2.18):
# - [重构] Fail2ban 安装 (3.1) 后立即进入配置流程 (设置端口/次数/时间)。
# - [重构] Fail2ban 修改配置 (3.2) 改为完全覆盖 jail.local 并重启，避免累积错误。
# - [新增] UFW 管理 (2) 增加删除规则功能 (可批量)。
# - [新增] UFW 首次启用 (2.1) 后增加批量开放额外端口的选项。
# - [尝试修复] 脚本开始时添加 stty sane 尝试解决 Backspace 输入 ^H 的问题。
# - [回退] 基于 V2.15 版本进行修改。
#
# 功能:
# 1.  **基础工具**: 安装常用软件包。
