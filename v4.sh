...
# ==============================================================================
# 服务器初始化与管理脚本 (v7.7 - Gemini-Mod)
#
# 更新日志 v7.7:
# 1. [智能修复] "添加域名"功能增强：如果在设置反代地址时只输入了端口号(如 30000)，
#    脚本会自动补全为 127.0.0.1:30000，防止 Nginx 启动失败。
# 2. [逻辑优化] 修复了 Nginx 重载失败时脚本依然提示"配置已应用"的显示 bug。
# 3. [保持] 包含 v7.6 的详细系统信息及所有历史修复。
# ==============================================================================
...
add_new_domain() {
    ...
    if confirm_action "是否需要自动配置 Nginx 反向代理?"; then
        ...
        if [[ -z "$LOCAL_PROXY_PASS" ]]; then 
            echo "地址为空，跳过 Nginx 配置"; setup_proxy="n"
        else
            # [v7.7] 智能补全 IP
            if [[ "$LOCAL_PROXY_PASS" =~ ^[0-9]+$ ]]; then
                echo -e "${YELLOW}[!] 检测到纯端口号，自动补全为 127.0.0.1:${LOCAL_PROXY_PASS}${NC}"
                LOCAL_PROXY_PASS="127.0.0.1:${LOCAL_PROXY_PASS}"
            fi
        fi
    fi
...
EOF
            ...
            if systemctl reload nginx; then
                ...
                echo -e "${RED}[✗] Nginx 重载失败！请检查配置：nginx -t${NC}"
            fi
        fi
