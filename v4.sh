...
# ==============================================================================
# 服务器初始化与管理脚本 (v7.8 - Gemini-Mod)
# 基于 v7.4 回滚，并集成 v7.7 的智能端口修复
#
# 更新日志 v7.8:
# 1. [回滚核心] 恢复 v7.4 的完整菜单结构和详细系统信息查询功能，拒绝过度精简。
# 2. [智能修复] 集成 v7.7 的 Nginx 反代地址智能补全 (输入端口号自动补全 127.0.0.1)。
# 3. [逻辑优化] 优化 Nginx 重载检测逻辑，配置错误时不再误报“配置已应用”。
# 4. [功能保持] 保留 Backspace 键修复、SSH 密码修改、证书自动续期 Hook 等所有核心功能。
# ==============================================================================
...
# Generated by server-manage.sh (v7.8) for ${DOMAIN}
...
EOF
    ...
    if systemctl reload nginx; then 
        ...
        echo -e "${RED}[✗] Nginx 重载失败！请检查配置文件或端口占用。${NC}"
        return 1
    fi
    return 0
}
...
setup_cron_jobs() {
    ...
    # [v7.8] 保持 v7.3 的增强 Hook，强制检测服务
    cat > "$DEPLOY_HOOK_SCRIPT" <<EOF
#!/bin/bash
# [v7.8] 增强版证书续期部署钩子 for ${DOMAIN}
# Certbot 续期成功后触发
...
if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet nginx; then
    log_hook "INFO: 检测到 Nginx 正在运行，尝试重载..."
    if systemctl reload nginx; then
        log_hook "SUCCESS: Nginx 重载成功。"
        RELOADED=1
    else
        log_hook "ERROR: Nginx 重载失败！"
    fi
fi
...
exit 0
EOF
    chmod +x "$DEPLOY_HOOK_SCRIPT"; echo -e "${GREEN}[✓] 部署钩子脚本 (v7.8) 创建成功。${NC}"
...
show_main_menu() {
    ...
    echo -e "\n${CYAN}================================================================${NC}"
    echo -e "${CYAN}     服务器管理脚本 (v7.8 - Gemini-Mod)     ${NC}"
    echo -e "${CYAN}================================================================${NC}"
